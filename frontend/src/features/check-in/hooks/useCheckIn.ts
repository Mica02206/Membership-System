...
export function useCheckIn() { const [result, setResult] = useState<MemberStatus | null>(null); const [error, setError] = useState(""); const [loading, setLoading] = useState(false);
  async function submit(memberId: string, action: "check-in" | "check-out" = "check-in") { setLoading(true); setError(""); setResult(null); try { setResult(await checkMember(memberId, action)); } catch (e) { setError(e instanceof Error ? e.message : "Unable to record activity"); } finally { setLoading(false); } }
